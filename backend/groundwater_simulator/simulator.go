...
type SimTask struct {
	ID        string
	Request   models.WaterLevelSimulationRequest
	Status    string
	Results   []models.WaterLevelSimulationResult
	Error     string
	CreatedAt time.Time
	UpdatedAt time.Time
}

type GroundwaterSimulator struct {
	aquiferParams  map[string]AquiferParams
	tasks          map[string]*SimTask
	taskMutex      sync.RWMutex
	taskChan       chan string
	resultChan     chan SimTask
}

func New() *GroundwaterSimulator {
	return &GroundwaterSimulator{
		aquiferParams: map[string]AquiferParams{
			"gravel": {
				Permeability:     0.01,
				SpecificYield:    0.25,
		...
		resultChan: make(chan SimTask, 100),
	}
}

func (s *GroundwaterSimulator) Start(ctx context.Context) {
	go s.run(ctx)
	log.Println("Groundwater Simulator: started")
}

func (s *GroundwaterSimulator) run(ctx context.Context) {
	for {
		select {
		case <-ctx.Done():
			log.Println("Groundwater Simulator: stopped")
			return
...
func (s *GroundwaterSimulator) processTask(taskID string) {
	s.taskMutex.RLock()
	...
	results := s.Simulate(task.Request)
...
func (s *GroundwaterSimulator) updateTaskResult(result SimTask) {
	s.taskMutex.Lock()
	s.tasks[result.ID] = &result
	s.taskMutex.Unlock()
}

func (s *GroundwaterSimulator) SubmitAsync(req models.WaterLevelSimulationRequest) string {
...
func (s *GroundwaterSimulator) GetTaskStatus(taskID string) (*SimTask, bool) {
	s.taskMutex.RLock()
...
func (s *GroundwaterSimulator) GetAquiferInfo(aquiferType string) models.AquiferInfo {
...
func (s *GroundwaterSimulator) getModelAssumptions() []string {
	return []string{
...
func (s *GroundwaterSimulator) getAquiferParams(aquiferType string) AquiferParams {
...
func (s *GroundwaterSimulator) GetDefaultScenarios() []models.WaterLevelScenario {
...
func (s *GroundwaterSimulator) Simulate(req models.WaterLevelSimulationRequest) []models.WaterLevelSimulationResult {
...
func (s *GroundwaterSimulator) simulateSingleScenario(
...
func (s *GroundwaterSimulator) calculateFlowRate(
...
func (s *GroundwaterSimulator) generateRecommendations(
	scenario models.WaterLevelScenario,
...
func generateTaskID() string {
	return "sim-" + intToStr(int(time.Now().UnixNano()))
}
