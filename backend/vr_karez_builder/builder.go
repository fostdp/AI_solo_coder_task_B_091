...
type VrKarezBuilder struct {
	mu       sync.RWMutex
	projects map[string]*models.VirtualDigProject
}

func New() *VrKarezBuilder {
	return &VrKarezBuilder{
		projects: make(map[string]*models.VirtualDigProject),
	}
}

func (b *VrKarezBuilder) generateID() string {
	buf := make([]byte, 8)
	rand.Read(buf)
	return hex.EncodeToString(buf)
}

func (b *VrKarezBuilder) GetDefaultTerrain() models.TerrainConfig {
...
func (b *VrKarezBuilder) GetDigGuide() models.DigGuide {
...
func (b *VrKarezBuilder) GetDesignTemplates() []models.DesignTemplate {
	terrain := b.GetDefaultTerrain()
...
func (b *VrKarezBuilder) GetQuickTips() []models.QuickTip {
...
func (b *VrKarezBuilder) SaveProject(req models.VirtualDigSaveRequest) (*models.VirtualDigProject, error) {
	...
	for i := range project.Channels {
		if project.Channels[i].ID == "" {
			project.Channels[i].ID = "ch_" + b.generateID()
		}
		if project.Channels[i].Name == "" {
			project.Channels[i].Name = "暗渠" + intToStr(i+1)
		}
		b.calculateChannelMetrics(&project.Channels[i], project.TerrainMap)
	}

	for i := range project.Shafts {
		if project.Shafts[i].ID == "" {
			project.Shafts[i].ID = "sh_" + b.generateID()
		}
		if project.Shafts[i].Name == "" {
			project.Shafts[i].Name = "竖井" + intToStr(i+1)
		}
		b.calculateShaftMetrics(&project.Shafts[i], project.TerrainMap)
	}

	project.Statistics = b.calculateStatistics(project.Channels, project.Shafts)
	project.SimulatedFlow = b.estimateFlow(project.Channels, project.Shafts, project.TerrainMap)
	project.Feasibility = b.evaluateFeasibility(project)
...
func (b *VrKarezBuilder) SimulateDesign(req models.VirtualDigSimulateRequest) (*models.VirtualDigProject, error) {
	return b.SaveProject(models.VirtualDigSaveRequest{
		ProjectName: "模拟项目_" + b.generateID(),
		Creator:     "system",
		TerrainMap:  req.TerrainMap,
		Channels:    req.Channels,
		Shafts:      req.Shafts,
...
func (b *VrKarezBuilder) GetProject(id string) (*models.VirtualDigProject, bool) {
	b.mu.RLock()
	defer b.mu.RUnlock()
...
func (b *VrKarezBuilder) ListProjects() []*models.VirtualDigProject {
	b.mu.RLock()
	defer b.mu.RUnlock()
	projects := make([]*models.VirtualDigProject, 0, len(b.projects))
...
func (b *VrKarezBuilder) DeleteProject(id string) bool {
	b.mu.Lock()
	defer b.mu.Unlock()
...
func (b *VrKarezBuilder) calculateChannelMetrics(ch *models.DigChannel, terrain models.TerrainConfig) {
	if len(ch.Points) < 2 {
		ch.Length = 0
		ch.Slope = 0
		return
...
func (b *VrKarezBuilder) calculateShaftMetrics(sh *models.DigShaft, terrain models.TerrainConfig) {
...
func (b *VrKarezBuilder) calculateStatistics(
	channels []models.DigChannel,
	shafts []models.DigShaft,
...
func (b *VrKarezBuilder) estimateFlow(
	channels []models.DigChannel,
	shafts []models.DigShaft,
	...
	for _, ch := range channels {
		...
		soilFactor := b.getSoilFlowFactor(terrain.SoilType)
		flowRate *= soilFactor
...
func (b *VrKarezBuilder) getSoilFlowFactor(soilType string) float64 {
	switch soilType {
	case "gravel":
		return 1.2
	case "sand":
...
func (b *VrKarezBuilder) evaluateFeasibility(project *models.VirtualDigProject) models.FeasibilityReport {
	issues := make([]models.FeasibilityIssue, 0)
	suggestions := make([]string, 0)

	hydraulicScore := b.evaluateHydraulics(project, &issues, &suggestions)
	geologicalScore := b.evaluateGeology(project, &issues, &suggestions)
	economicScore := b.evaluateEconomics(project, &issues, &suggestions)
...
func (b *VrKarezBuilder) evaluateHydraulics(
	project *models.VirtualDigProject,
...
func (b *VrKarezBuilder) evaluateGeology(
	project *models.VirtualDigProject,
	issues *[]models.FeasibilityIssue,
	suggestions *[]string,
) float64 {
...
func (b *VrKarezBuilder) evaluateEconomics(
	project *models.VirtualDigProject,
	issues *[]models.FeasibilityIssue,
	suggestions *[]string,
) float64 {
